...
# NO pydub_patch.py, NO audioop errors, NO extra files
...
# ——— PATCH PYDUB TO AVOID audioop (Python 3.13 FIX) ———
# audioop is removed in Python 3.13 — we don't use it anyway
import sys
if "audioop" in sys.modules:
    del sys.modules["audioop"]
# Monkey-patch pydub.utils to skip audioop
from pydub import utils
utils.audioop = None  # pydub will fall back to numpy (which we use)
# ———————————————————————————————————————————————————

# ——— PWA: OFFLINE + HOMESCREEN INSTALL ———
...
def get_drum(drum_type, base_freq=60):
    ...
    elif drum_type == "woodblock": wave = np.sin(2*np.pi*800*t) * np.exp(-t*15)
...
def add_polyrhythm_layer(audio, bpm, drum_type, base_freq, duration_sec, pattern="4/4"):
    ...
    if pattern == "3:2": beats = [0, 1, 2, 0.5, 1.5]
    elif pattern == "5:4": beats = [0, 0.8, 1.6, 2.4, 0.4, 1.2, 2.0]
    elif pattern == "7:8": beats = [0, 0.875, 1.75, 2.625, 0.4375, 1.3125, 2.1875]
    else: beats = [0, 1, 2, 3]
    ...
    for layer in layers:
        ...
        if layer.get("drum", False):
            if layer.get("shamanic", False):
                ...
                layer_audio = add_polyrhythm_layer(
                    layer_audio,
                    layer.get("bpm", 60),
                    layer.get("drum_type", "bass_drum"),
                    layer.get("drum_freq", 60),
                    duration_sec,
                    layer.get("pattern", "4/4")
                )
...
# ——— STREAMLIT UI ———
st.title("Sacred Sound Codex — DIDGERIDOO + SHAMANIC DRUMS + LIVE PIANO")
st.markdown("### *All Earth Tones. All Rhythms. All Layers.*")
...
with tab1:
    if 'layers' not in st.session_state:
        st.session_state.layers = [{
            ...
            "drum": False, "bpm": 60, "drum_type": "bass_drum", "drum_freq": 60, "pattern": "4/4", "shamanic": False
        }]

    def add_layer(): st.session_state.layers.append({**st.session_state.layers[0]})
    def remove_layer(i): st.session_state.layers.pop(i)
    ...
    for i, layer in enumerate(st.session_state.layers):
        with st.expander(f"Layer {i+1}", expanded=i==0):
            ...
            with col2:
                ...
                if layer["drum"]:
                    layer["shamanic"] = st.checkbox("Shamanic Pattern", key=f"sh{i}")
                    layer["bpm"] = st.slider("BPM", 0, 300, 60, key=f"bpm{i}")
                    drum_options = ["bass_drum","snare","woodblock","djembe","conga","bongo","shaman_drum","shaker","hi_hat"]
                    layer["drum_type"] = st.selectbox("Drum", drum_options, key=f"dt{i}")
                    layer["drum_freq"] = st.number_input("Drum Freq", 20, 200, 60, key=f"df{i}")
                    if layer["shamanic"]:
                        layer["pattern"] = st.selectbox("Pattern", ["heartbeat", "trance", "journey"], key=f"p{i}")
                    else:
                        layer["pattern"] = st.selectbox("Pattern", ["4/4", "3:2", "5:4", "7:8"], key=f"p{i}")
...
with tab2:
    ...
    tone_preset = st.selectbox("Piano Tone", [
        "sine","guitar","piano","crystal","tibetan","native_flute","didgeridoo"
    ])
    
    st.markdown("### Live Piano Keys (Harmonized)")
