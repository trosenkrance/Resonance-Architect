# app.py — SACRED SOUND CODEX v3 — FULLY RESTORED + PYTHON 3.13 SAFE + NO SYNTAX ERRORS
...
# Apply patch BEFORE pydub import
sys.modules['audioop'] = NumPyAudioop()

# Now import pydub safely
...
def get_drum(drum_type, base_freq=60):
    ...
    if drum_type == "bass_drum": 
        ...
        wave = np.sin(2*np.pi*base_freq*t) * np.exp(-t*6) + 0.3*np.sin(2*np.pi*base_freq*3*t)*np.exp(-t*8)
    elif drum_type == "conga": 
        wave = np.sin(2*np.pi*base_freq*t) * np.exp(-t*5) + 0.2*np.sin(2*np.pi*base_freq*2*t)*np.exp(-t*7)
    elif drum_type == "bongo": 
        wave = np.sin(2*np.pi*base_freq*t) * np.exp(-t*10)
    elif drum_type == "shaker": 
        wave = np.random.uniform(-1, 1, len(t)) * np.exp(-t*20)
    elif drum_type == "hi_hat": 
        wave = np.random.uniform(-1, 1, len(t)) * np.exp(-t*30)
...
def piano_keyboard(base_freq=444, tone_preset="sine"):
    ...
    for i, (note, freq) in enumerate(zip(notes, freqs)):
        with cols[i]:
            key = f"key_{i}_{base_freq}_{tone_preset}"
            if st.button(note, key=key):
...
def generate_full_track(layers, duration_min):
    ...
    for layer in layers:
        ...
        if layer.get("binaural", 0) > 0:
            beat_freq = layer["binaural"]
            left = layer_audio
            right = layer_audio.low_pass_filter(beat_freq + 100).high_pass_filter(beat_freq - 100).apply_gain(-3)
...
# ——— UI ———
st.title("Sacred Sound Codex — FULL VERSION")
st.markdown("### *Didgeridoo • Shamanic Drums • Native Flute • Live Piano • Cymatics • PWA*")
...
with tab1:
    ...
    for i, layer in enumerate(st.session_state.layers):
        with st.expander(f"Layer {i+1}", expanded=(i==0)):
            ...
            with col1:
                freq_input = st.text_input("Freqs (comma sep)", "528", key=f"f{i}")
                ...
                layer["binaural"] = st.slider("Binaural Beat (Hz)", 0.0, 40.0, 6.0, 0.1, key=f"b{i}")
            with col2:
                layer["drum"] = st.checkbox("Add Drum Layer", key=f"d{i}")
                if layer["drum"]:
                    layer["bpm"] = st.slider("BPM", 30, 300, 60, key=f"bpm{i}")
                    layer["drum_type"] = st.selectbox("Drum Type", [
                        "bass_drum","snare","shaman_drum","djembe","conga","bongo","shaker","hi_hat"
                    ], key=f"dt{i}")
                    layer["drum_freq"] = st.number_input("Base Freq", 20, 200, 60, key=f"df{i}")
                    layer["pattern"] = st.selectbox("Pattern", ["heartbeat", "trance", "journey", "4/4"], key=f"p{i}")

    st.button("Add Layer", on_click=add_layer)
    
    if st.button("Generate Full Track", type="primary"):
        with st.spinner("Building sacred sound..."):
            ...
            st.download_button(
                "Download WAV",
                data=wav,
                file_name=f"codex_{int(time.time())}.wav",
                mime="audio/wav"
            )

with tab2:
    base_freq = st.number_input("Base Frequency", 100, 1000, 444, key="base_freq")
    tone_preset = st.selectbox("Piano Tone", [
        "sine","piano","crystal","tibetan","native_flute","didgeridoo"
    ], key="tone_preset")
    
    st.markdown("### Live Harmonic Piano")
    pressed = piano_keyboard(base_freq, tone_preset)
    for tone in pressed:
        wav_out = io.BytesIO()
        tone.export(wav_out, format="wav")
        wav_out.seek(0)
        st.audio(wav_out, format="audio/wav", autoplay=True)
...
Now launch from your home screen — full screen, offline-ready.
""")sys.modules['audioop'] = NumPyAudioop()
from pydub import AudioSegment
...
def get_drum(drum_type, base_freq=60):
    ...
    if drum_type == "bass_drum": wave = np.sin(2*np.pi*base_freq*t) * np.exp(-t*8)
        ...
        wave += 0.2 * np.sin(2*np.pi*base_freq*3.2*t) * np.exp(-t*8)
        wave *= np.exp(-t*0.5)
    elif drum_type == "djembe": wave = np.sin(2*np.pi*base_freq*t) * np.exp(-t*6) + 0.3*np.sin(2*np.pi*base_freq*3*t)*np.exp(-t*8)
    elif drum_type == "conga": wave = np.sin(2*np.pi*base_freq*t) * np.exp(-t*5) + 0.2*np.sin(2*np.pi*base_freq*2*t)*np.exp(-t*7)
    elif drum_type == "bongo": wave = np.sin(2*np.pi*base_freq*t) * np.exp(-t*10)
    elif drum_type == "shaker": wave = np.random.uniform(-1, 1, len(t)) * np.exp(-t*20)
    elif drum_type == "hi_hat": wave = np.random.uniform(-1, 1, len(t)) * np.exp(-t*30)
...
def piano_keyboard(base_freq=444, tone_preset="sine"):
    ...
    for i, (note, freq) in enumerate(zip(notes, freqs)):
        with cols[i]:
            if st.button(note, key=f"key_{i}_{base_freq}_{tone_preset}"):
                tone = get_tone(freq, 2.0, tone_preset)
                pressed.append(tone)
                st.session_state.piano_queue.put(tone)
    return pressed
...
with tab1:
    ...
    for i, layer in enumerate(st.session_state.layers):
        with st.expander(f"Layer {i+1}", expanded=i==0):
            ...
            with col1:
                ...
                layer["binaural"] = st.slider("Binaural Beat (Hz)", 0.0, 40.0, 6.0, 0.1, key=f"b{i}")
            with col2:
                layer["drum"] = st.checkbox("Drum Layer", key=f"d{i}")
                if layer["drum"]:
                    layer["bpm"] = st.slider("BPM", 30, 300, 60, key=f"bpm{i}")
                    layer["drum_type"] = st.selectbox("Drum", ["bass_drum","snare","shaman_drum","djembe","conga","bongo","shaker","hi_hat"], key=f"dt{i}")
                    layer["drum_freq"] = st.number_input("Base Freq", 20, 200, 60, key=f"df{i}")
                    layer["pattern"] = st.selectbox("Pattern", ["heartbeat", "trance", "journey", "4/4"], key=f"p{i}")
    ...
    if st.button("Generate Full Track", type="primary"):
        with st.spinner("Building sacred sound..."):
            ...
            st.audio(wav, format="audio/wav")
            st.download_button("Download WAV", wav, f"codex_{int(time.time())}.wav", "audio/wav")

with tab2:
    base_freq = st.number_input("Base Frequency", 100, 1000, 444)
    tone_preset = st.selectbox("Piano Tone", ["sine","piano","crystal","tibetan","native_flute","didgeridoo"])
    st.markdown("### Live Harmonic Piano")
    pressed = piano_keyboard(base_freq, tone_preset)
    for tone in pressed:
    ...
    with colr2:
        if st.button("Stop & Download"):
            if st.session_state.recording:
                ...
                st.session_state.recorded_audio.export(wav, format="wav")
                wav.seek(0)
                st.download_button("Download Recording", wav, "piano_recording.wav", "audio/wav")
                st.success("Saved!")
