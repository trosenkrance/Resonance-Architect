...
# ——— TONE GENERATOR (DIDGERIDOO + NATIVE FLUTE + TIBETAN) ———
@st.cache_data
def get_tone(freq, duration, preset="sine"):
    ...
    if preset == "sine": wave = np.sin(2*np.pi*freq*t)
        ...
        # Deep drone + formant + circular breathing modulation
        drone = np.sin(2*np.pi*freq*t)
        overtone = 0.4 * np.sin(2*np.pi*freq*2*t + 0.3)
        breath_mod = 1 + 0.3 * np.sin(2*np.pi*0.5*t)  # slow breath pulse
...
def add_shamanic_drum_layer(audio, bpm, drum_type, base_freq, duration_sec, pattern="heartbeat"):
    ...
    if pattern == "heartbeat":
        beats = [0, 0.6, 1.6, 2.2]  # Strong-weak, Strong-weak
    elif pattern == "trance":
        ...
        with st.expander(f"Layer {i+1}", expanded=i==0):
            ...
            with col2:
                ...
                if layer["drum"]:
                    ...
                    drum_options = ["bass_drum","snare","shaman_drum","djembe","conga","bongo","shaker","hi_hat"]
                    layer["drum_type"] = st.selectbox("Drum", drum_options, key=f"dt{i}")
                    layer["drum_freq"] = st.number_input("Drum Freq", 20, 200, 60, key=f"df{i}")
                    if layer["shamanic"]:
                        layer["pattern"] = st.selectbox("Shamanic", ["heartbeat", "trance", "journey"], key=f"p{i}")
                    else:
